        ...
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="rounded-[32px] bg-[#201e1d] p-5 sm:p-8">
                <div class="rounded-[28px] bg-[#474238] p-6 shadow-[0_1px_2px_rgba(46,43,37,0.14)]">
                    <form method="POST" action="{{ route('income-expectations.update', $incomeExpectation) }}" class="space-y-4">
                        @method('PUT')
                        @include('income-expectations._form')

                        <div class="flex justify-end gap-3 pt-1">
                            <a href="{{ route('income-expectations.index') }}" class="inline-flex items-center rounded-full border border-[#f9f4ed]/20 px-5 py-2.5 text-xs font-semibold uppercase tracking-widest text-[#f9f4ed]/70 transition hover:bg-[#f9f4ed]/10">Cancel</a>
                            <button type="submit"
