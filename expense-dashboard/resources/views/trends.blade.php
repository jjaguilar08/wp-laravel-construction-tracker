        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="rounded-[32px] bg-[#201e1d] p-5 sm:p-8 space-y-5">
                <div class="rounded-[28px] bg-[#474238] p-6 shadow-[0_1px_2px_rgba(46,43,37,0.14)]">
                    <p class="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#f9f4ed]/60">Total Spend, Last 6 Months</p>
                    <canvas id="trendChart" class="mt-4"></canvas>
                </div>

                <div class="rounded-[28px] bg-[#474238] p-6 shadow-[0_1px_2px_rgba(46,43,37,0.14)]">
                    <p class="mb-4 text-[10px] font-semibold uppercase tracking-[0.1em] text-[#f9f4ed]/60">Monthly Totals</p>
                    ...
                    <div class="space-y-3 sm:hidden">
                        @foreach ($monthlyTotals as $entry)
                            <div class="flex items-center justify-between rounded-2xl border border-[#f9f4ed]/10 bg-[#2e2b25] p-4 text-sm">
                                <span class="font-medium text-[#f9f4ed]">{{ $entry['month']->format('F Y') }}</span>
                                <span class="font-semibold text-[#ffc6a5]">${{ number_format($entry['total'], 2) }}</span>
                            </div>
                        @endforeach
                    </div>
                                ...
                                <tbody class="divide-y divide-[#f9f4ed]/8">
                                    @foreach ($monthlyTotals as $entry)
                                        <tr class="hover:bg-[#f9f4ed]/[0.06]">
                                            <td class="whitespace-nowrap px-4 py-3 text-[#f9f4ed]/70">{{ $entry['month']->format('F Y') }}</td>
                                            <td class="px-4 py-3 text-right font-semibold text-[#ffc6a5]">${{ number_format($entry['total'], 2) }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
