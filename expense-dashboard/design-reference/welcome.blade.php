...
<head>
    ...
    {{-- NOTE for Claude Code: reuse the exact Bunny Fonts <link> already in
         layouts/app.blade.php / layouts/guest.blade.php (Figtree + Caprasimo)
         instead of adding a second font-loading mechanism here. --}}
    <link rel="preconnect" href="https://fonts.bunny.net">
    <link href="https://fonts.bunny.net/css?family=figtree:400,500,600,700|caprasimo:400" rel="stylesheet" />
...
<body class="antialiased bg-[#201e1d] text-[#f9f4ed]">
    ...
    <header class="border-b border-[#f9f4ed]/10">
        <div class="max-w-6xl mx-auto px-6 py-5 flex items-center justify-between">
            ...
            <nav class="flex items-center gap-3">
                @auth
                    <a href="{{ route('dashboard') }}" class="text-sm font-semibold px-5 py-2 rounded-full bg-[#c1662f] text-[#f9f4ed]">
                        Go to dashboard
                    </a>
                @else
                    <a href="{{ route('login') }}" class="text-sm px-4 py-2 rounded-full text-[#f9f4ed]/60 hover:text-[#f9f4ed]">
                        Log in
                    </a>
                    @if (Route::has('register'))
                        <a href="{{ route('register') }}" class="text-sm font-semibold px-5 py-2 rounded-full bg-[#c1662f] text-[#f9f4ed]">
                            Get started
                        </a>
                    @endif
                @endauth
    ...
    <section class="max-w-6xl mx-auto px-6 pt-20 pb-16 text-center">
        <p class="text-sm tracking-widest uppercase mb-4 text-[#c1662f]">Personal finance, kept simple</p>
        ...
        <div class="flex items-center justify-center gap-4 flex-col sm:flex-row">
            @guest
                @if (Route::has('register'))
                    <a href="{{ route('register') }}" class="w-full sm:w-auto text-center font-semibold px-8 py-3 rounded-full bg-[#c1662f] text-[#f9f4ed]">
                        Create free account
                    </a>
                @endif
            ...
            @else
                <a href="{{ route('dashboard') }}" class="w-full sm:w-auto text-center font-semibold px-8 py-3 rounded-full bg-[#c1662f] text-[#f9f4ed]">
                    Go to dashboard
                </a>
            @endguest
        </div>
    ...
    <section class="max-w-5xl mx-auto px-6 pb-24">
        <div class="bg-[#474238] border border-[#f9f4ed]/10 rounded-2xl p-6 sm:p-8 shadow-2xl">
            <div class="flex items-center justify-between mb-6">
                ...
                <span class="font-['Caprasimo'] text-xs px-3 py-1 rounded-full bg-[#2e2b25] text-[#c1662f]">Sample data</span>
            </div>
                ...
                <div class="bg-[#2e2b25] rounded-xl p-4">
                    ...
                    <div class="space-y-2">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex-1 h-2 rounded-full bg-[#f9f4ed]/10"><div class="h-2 rounded-full bg-[#c1662f]" style="width: 85%"></div></div>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="text-xs w-20 text-[#f9f4ed]/60">Food</span>
                            <div class="flex-1 h-2 rounded-full bg-[#f9f4ed]/10"><div class="h-2 rounded-full bg-[#c1662f]" style="width: 55%"></div></div>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="text-xs w-20 text-[#f9f4ed]/60">Bills</span>
                            <div class="flex-1 h-2 rounded-full bg-[#f9f4ed]/10"><div class="h-2 rounded-full bg-[#c1662f]" style="width: 40%"></div></div>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="text-xs w-20 text-[#f9f4ed]/60">Other</span>
                            <div class="flex-1 h-2 rounded-full bg-[#f9f4ed]/10"><div class="h-2 rounded-full bg-[#c1662f]" style="width: 22%"></div></div>
                        </div>
                    </div>
                </div>
                <div class="bg-[#2e2b25] rounded-xl p-4">
                    <p class="text-xs mb-3 text-[#f9f4ed]/60">Savings progress</p>
                    <div class="h-2 rounded-full mb-2 bg-[#f9f4ed]/10"><div class="h-2 rounded-full bg-[#c1662f]" style="width: 72%"></div></div>
    ...
    <section class="max-w-6xl mx-auto px-6 pb-24">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-[#474238] border border-[#f9f4ed]/10 rounded-xl p-6">
                <div class="w-10 h-10 rounded-full flex items-center justify-center mb-4 bg-[#2e2b25] text-[#c1662f]">
    ...
    @guest
        <section class="max-w-6xl mx-auto px-6 pb-24">
            <div class="bg-[#474238] border border-[#f9f4ed]/10 rounded-2xl p-10 sm:p-14 text-center">
                ...
                @if (Route::has('register'))
                    <a href="{{ route('register') }}" class="inline-block font-semibold px-8 py-3 rounded-full bg-[#c1662f] text-[#f9f4ed]">
                        Create free account
                    </a>
                @endif
            </div>
