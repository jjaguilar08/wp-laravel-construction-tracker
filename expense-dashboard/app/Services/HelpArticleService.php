...
/**
 * Fetches help articles from the WordPress "Construction Tracker" plugin's
 * REST endpoint (`GET /wp-json/wp-tracker/v1/articles`), documented in
 * `PROJECT_NOTES.md`.
 */
class HelpArticleService
{
    /** Cache key the decoded WP response is stored under. */
    private const CACHE_KEY = 'help_articles';

    /** How long a successful response is cached before WordPress is hit again. */
    private const CACHE_TTL_SECONDS = 60;

    /**
     * Get all help articles, served from a short-lived cache where possible.
     *
     ...
     *                    `id`, `title`, `content`, `slug`, `featured_image`.
     *
    ...
    public function getArticles(): Collection
    {
        ...
        $articles = Cache::remember(self::CACHE_KEY, self::CACHE_TTL_SECONDS, function () {
            try {
                return Http::baseUrl(config('services.wp_tracker.base_url'))
                    ->timeout(5)
                    ->get('/wp-json/wp-tracker/v1/articles')
                    ->throw()
                    ->json();
            } catch (ConnectionException|RequestException $e) {
                Log::error('Failed to fetch help articles from WordPress: '.$e->getMessage());
